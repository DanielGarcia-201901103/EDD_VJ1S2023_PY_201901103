...
type NodoMAdyacencia struct {
	Siguiente *NodoMAdyacencia
	Abajo     *NodoMAdyacencia
	Valor     string
}
...
func (g *Grafo) insertarC(padre string, hijo string, filtro string) { // cliente, imagen
	nuevoNodo := &NodoMAdyacencia{Valor: hijo}
	if g.Principal != nil && padre == g.Principal.Valor {
		...
		if aux != nil {
			nuevoNodo.Siguiente = &NodoMAdyacencia{Valor: filtro}
			aux.Siguiente = nuevoNodo
		}
	}
}
...
func (g *Grafo) insertarF(padre string) {
	nuevoNodo := &NodoMAdyacencia{Valor: padre}
	if g.Principal == nil {
